...
const Table = ({ columns, rows }: { columns: Column[]; rows: DataRow[] }) => {
  ...
  return (
    <div>
      <div className="lg:w-[1060px] rounded-md p-6">
        <input
          type="search"
          placeholder="Search by token address..."
          className="w-full  rounded-md py-2 px-2 dark:text-white text-black font-bold border border-green-500"
          value={searchQuery}
        ...
        <div className="mt-4 overflow-x-auto rounded-lg">
          <table className="min-w-full">
            <thead className="dark:dark:bg-[#323333]">
              <tr>
                {columns.map((column) => (
                  <th
                    key={column.id}
                  ...
                  <tr key={row.token}>
                    {columns.map((column) => (
                      ...
                      >
                        {column.id === "view" ? (
                          <button
                            className="border rounded-md px-2 py-1 text-sm bg-green-600 hover:bg-green-500 text-white"
                            onClick={() => {
        ...
        <div className="flex justify-between items-center mt-4 ">
          ...
          <div className="space-x-6">
            <select
              className="border rounded-md py-1 text-sm mr-10"
              value={rowsPerPage}
        ...
        <div className="flex mt-4 gap-4">
          <button
            className="border rounded-md px-4 py-1 text-sm bg-green-600 hover:bg-green-500 text-white"
...
const Index = () => {
  ...
  console.log(dataPinkMyLock + "my lock");
  ...
  console.log(lastIndexOfDataMainPresale + "last index"); //output: 2

  const idWeb2PinkLock = title.map((item: any) => item.id);
  console.log(idWeb2PinkLock); //output: [1,2,3]

  const idWeb3Auction = lastIndexOfDataMainPresale + 1;
  console.log(idWeb3Auction + "id web3 auction"); //output: 3
  ...
  return (
    <div className="flex justify-center items-center mt-4">
      <div className="dark:bg-[#242525] lg:w-[1060px] bg-stone-50 rounded-md p-6">
