...
import Bottom from "@/components/details/Multichain/PolygonMumbai/PinkLock/record/Bottom";
import MiddleBox from "@/components/details/Multichain/PolygonMumbai/PinkLock/record/Middle";
import TopBox from "@/components/details/Multichain/PolygonMumbai/PinkLock/record/Top";
...
import Button from "@/components/details/Multichain/PolygonMumbai/PinkLock/record/Button";
interface Props {
  title: string;
}
const Record = () => {
  ...
  useEffect(() => {
    axios
      .get(`/fetch-data-pinklock/PolygonMumbai`)
      .then((response: any) => {
  ...
  const cycle = dataMainPresale[dataIndex]?.vestingPeriod;
  ...
  //button logic
  const banificiary = dataMainPresale[dataIndex]?.beneficiary;
  return (
    <div>
      <TopBox EndTime={dataMainPresale[dataIndex]?.unlockTime} />
      <MiddleBox
      ...
      <Bottom
        title={titles}
        totalAmountLocked={dividedAmount[dataIndex]}
        totalValueLocked={amount}
        owner={dataMainPresale[dataIndex]?.beneficiary}
        unlockDate={unlockTime}
        tokenSymbol={tokenSymbol}
      />
      <div className="flex justify-center">
        <Button argsValue={dataIndex} beneficiary={banificiary} />
      </div>
    </div>
  );
};
