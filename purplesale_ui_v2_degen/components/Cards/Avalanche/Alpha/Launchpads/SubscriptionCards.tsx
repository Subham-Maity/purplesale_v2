...
const SubscriptionCards = ({
  ...
  const handleHeartButtonClick = async (
    ...
    const postData = {
      Name: name,
      Symbol: symbol,
      Link: `/details/Alpha/subscription/${id}`,
      WalletAddress: address,
      imgHref: imgHref,
    };
    try {
      await axios.post("/cart/Avalanche", postData);
      setIsCardAdded(true);
  ...
  return (
    <div className=" h-[527px] w-[346px] mb-12 rounded-2xl bg-gradient-to-tr from-[#2020aa] to-[#6161b3] shadow-black shadow-2xl border-blue-600/25 border ">
      ...
      <div className="px-3 mb-8">
        <h5 className="mb-2 flex justify-start text-2xl font-bold tracking-tight text-gray-300 dark:text-white">
          {name}
        </h5>
        <p className="mb-3 flex justify-start font-normal text-gray-300 dark:text-gray-400">
          Subscription
        </p>
      </div>
        ...
        <div className="flex justify-between">
          ...
          <div className=" flex justify-between gap-3">
            <button
              ...
              onClick={() => {
                router.push({
                  pathname: "/details/Alpha/subscription/[id]",
                  query: { id: id },
                });
              }}
            >
