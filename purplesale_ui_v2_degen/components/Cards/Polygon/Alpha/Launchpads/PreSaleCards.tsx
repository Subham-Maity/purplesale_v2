...
const PreSaleCards = ({
  ...
  const handleHeartButtonClick = async (
    ...
    const postData = {
      Name: name,
      Symbol: symbol,
      Link: `/details/Alpha/launchpad/${id}`,
      WalletAddress: address,
      imgHref: imgHref,
    };
    try {
  ...
  return (
    <div className="h-[527px] w-[346px] mb-12 rounded-2xl bg-gradient-to-tr from-[#2020aa] to-[#6161b3] shadow-black shadow-2xl border-blue-600/25 border ">
      ...
      <div className="px-3 mb-8 ">
        <h5 className="mb-2 text-2xl flex justify-start font-semibold whitespace-nowrap tracking-tight text-white">
          {name}
        </h5>
        <p className=" flex justify-start mb-3 font-normal text-lg text-gray-200">
          1 {currency} = {description} {symbolTokenPrivateSales}
        </p>
      </div>
      <div className="px-4">
        <p className="mb-1 flex justify-start text-sm font-bold text-white">
        ...
        <div className="flex justify-between">
          ...
          <div className=" flex justify-between gap-3">
            <button
              ...
              onClick={() => {
                router.push({
                  pathname: "/details/Alpha/launchpad/[id]",
                  query: { id: id },
                });
              }}
            >
